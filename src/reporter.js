...
async function processArticles() {
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    if (insertError) {
      ...
      // TTS DISABLED - audio will be generated by the app when user plays the report
      // This saves OpenAI TTS costs. Re-enable later via GitHub Actions if needed.
      // const ttsText = `${report.title}. ${report.content}`;
      // const audioBuffer = await generateTTSAudio(ttsText);
      // if (audioBuffer) {
      //   const audioUrl = await uploadAudioToStorage(audioBuffer, insertedReport.id);
      //   if (audioUrl) {
      //     await supabase.from('reports').update({ audio_url: audioUrl }).eq('id', insertedReport.id);
      //   }
      // }
